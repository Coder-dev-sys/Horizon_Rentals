<?php
session_start();

// Establish database connection.
$con = mysqli_connect("localhost", "root", "", "horizonrental");

// Checking connection
if ($con === false) {
?>
    <script>
        alert("<?php echo "ERROR: Could not connect. " . mysqli_connect_error(); ?>");
    </script>
<?php
}

// Insertng the data into database
if (isset($_POST['submit'])) {
    $fullName = $_POST['fullName'];
    $drivingLicNo = $_POST['licenseNo'];
    $userEmail = $_POST['userEmail'];
    $phoneNum = $_POST['phone'];
    $fromDate = $_POST['from_date'];
    $toDate = $_POST['to_date'];
    $description = $_POST['description'];
    $query = "insert into tblbooking(fullName,drivingLicNo,userEmail,phoneNum,fromDate,toDate,description)values('$fullName','$drivingLicNo','$userEmail','$phoneNum','$fromDate','$toDate','$description')";
    $run = mysqli_query($con, $query) or die("Can't insert data into database" . mysqli_error($con));
    if ($run) {
        echo "<script>alert('Booking Done Successfully');</script>";
    } else {
        echo "<script>alert('Booking Unsuccessful');</script>";
    }
}
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" type="image/png" href="/Horizon_Rentals/assets/Logo/Bike.png" />
    <link rel="stylesheet" href="/Horizon_Rentals/assets/css/style.css" />
    <link rel="stylesheet" href="/Horizon_Rentals/assets/css/responsive.css" />
    <title>Horizon Rentals</title>
    <style>
        .nz-contact-title {
            font-size: 2.5rem;
        }

        .nz-submit-btn {
            align-self: center;
        }
    </style>
</head>

<body>
    <!-- Page Loader -->
    <div class="page-loader">
        <div class="loader-content">
            <img src="/Horizon_Rentals/assets/Logo/Bike.png" alt="Horizon Rentals" class="loader-logo">
            <div class="dots"></div>
        </div>
    </div>

    <!-- Navbar Section -->
    <header>
        <a href="#" class="header-logo"><img src="/Horizon_Rentals/assets/Logo/final-removedbg.png" alt="Horizon Rentals logo" /></a>
        <div id="menu-icon">
            <label class="bar" for="check">
                <input type="checkbox" id="check" />
                <span class="top"></span>
                <span class="middle"></span>
                <span class="bottom"></span>
            </label>
        </div>
        <ul class="navbar">
            <li><a href="/Horizon_Rentals/#">Home</a></li>
            <li><a href="/Horizon_Rentals/#vehicles">Vehicles</a></li>
            <li><a href="/Horizon_Rentals/#about">About Us</a></li>
            <li><a href="/Horizon_Rentals/includes/contact_faq.php#faq">FAQ's</a></li>
        </ul>
    </header>

    <!-- Booking Section -->
    <section class="nz-contact" id="contact_us">
        <div class="nz-contact-container">
            <div class="nz-contact-card">
                <div class="nz-contact-form-container">
                    <h2 class="nz-contact-title">Booking</h2>
                    <form id="contactForm" class="nz-contact-form" method="post" action="booking-page.php">
                        <div class="nz-form-row">
                            <div class="nz-form-group">
                                <label for="firstName">Full Name *</label>
                                <input type="text" id="fullName" name="fullName" class="nz-form-control" placeholder="Enter your full name" pattern="[A-Za-z\s]+" required>
                            </div>
                            <div class="nz-form-group">
                                <label for="lastName">Driving License No. *</label>
                                <input type="text" id="licenseNo" name="licenseNo" class="nz-form-control" placeholder="DL0420230001234" pattern="[A-Z]{2}[0-9]{2}[0-9]{11}" required>
                            </div>
                        </div>

                        <div class="nz-form-row">
                            <div class="nz-form-group">
                                <label for="email">Email *</label>
                                <input type="email" id="email" name="userEmail" class="nz-form-control" placeholder="example@company.com" required>
                            </div>
                            <div class="nz-form-group">
                                <label for="phone">Phone Number *</label>
                                <input type="tel" id="phone" name="phone" class="nz-form-control" placeholder="9876543210" pattern="[0-9]{10}" required>
                            </div>
                        </div>

                        <div class="nz-form-row">
                            <div class="nz-form-group">
                                <label for="from_date">From Date *</label>
                                <input type="date" class="nz-form-control" name="from_date" id="pickup-date" required />
                            </div>
                            <div class="nz-form-group">
                                <label for="to_date">To Date *</label>
                                <input type="date" class="nz-form-control" name="to_date" id="dropoff-date" required />
                            </div>
                        </div>

                        <div class="nz-form-group nz-textarea-group">
                            <label for="message">Description</label>
                            <textarea id="message" class="nz-form-control" name="description" placeholder="Type your message here..."
                                rows="5"></textarea>
                        </div>
                        <button type="submit" name="submit" class="nz-submit-btn">Book</button>
                    </form>
                </div>
            </div>
        </div>
    </section>

    <script src="/Horizon_Rentals/assets/js/script.js"></script>
</body>

</html>